...
import { getAcademicProjects, getAcademicProjectBySlug } from "@/lib/content";
...
export async function generateStaticParams() {
  return getAcademicProjects().map((p) => ({ slug: p.slug }));
}


export default async function AcademicProjectPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  const { slug } = await params;
  const result = await getAcademicProjectBySlug(slug);
  if (!result) notFound();

  const { frontmatter: p, source } = result;

  const allProjects = getAcademicProjects();
  const related = allProjects.filter((proj) => proj.slug !== slug).slice(0, 2);

  const infoItems = [
    { label: "Type", value: p.context },
  ...
  return (
    ...
    >
      ...
      {/* Back */}
      <div className="mt-16">
        <Link href="/#academic" className="inline-flex items-center gap-2 text-sm text-[#78716c] hover:text-[#1c1917] transition-colors font-sans">
