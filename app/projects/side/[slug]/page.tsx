...
import { getSideProjects, getSideProjectBySlug } from "@/lib/content";
...
export async function generateStaticParams() {
  return getSideProjects().map((p) => ({ slug: p.slug }));
}
...
export default async function SideProjectPage({
  ...
  const result = await getSideProjectBySlug(slug);
  if (!result) notFound();

  const { frontmatter: p, source } = result;
  const related = getSideProjects().filter((proj) => proj.slug !== slug).slice(0, 2);
  ...
  return (
    ...
    >
      ...
      <div className="mb-12">
        ...
        <p className="text-lg text-[#57534e] mb-6 leading-relaxed font-sans">
          {p.summary}
        </p>

        {/* Metadata row */}
        {(p.role || p.timeToMvp || p.status) && (
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm mb-5">
            {[
              { label: "Role", value: p.role },
              { label: "Time to MVP", value: p.timeToMvp },
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {p.url && (
            <a
              ...
              className="px-2 py-1 text-xs font-mono rounded bg-[#5b7a52]/10 text-[#5b7a52] hover:bg-[#5b7a52]/20 transition-colors"
            >
              Visit →
            </a>
          )}
      ...
      {related.length > 0 && (
        <div className="mt-24 pt-12 border-t border-[#e7e5e4]">
          ...
          <div className="space-y-6">
            {related.map((proj) => (
              <Link key={proj.slug} href={`/projects/side/${proj.slug}`} className="block group">
      ...
      {/* Back */}
      <div className="mt-16">
        <Link href="/#side" className="inline-flex items-center gap-2 text-sm text-[#78716c] hover:text-[#1c1917] transition-colors font-sans">
